$(document).ready(function() {
    ...
    var bot = '<div class="chatCont" id="chatCont" style="background-color: whitesmoke; border-radius: 5px; border: 0; margin-right: 1%">' +
        ...
        // '<img src="logo.png" class="bot_p_img" style="box-shadow: 0px 0px 5px #5a0421;">' +
        '<div class="exit" style="float: right; margin-right: 2%; margin-top: 5%;">' +
        '<i class="fa fa-times-circle-o" aria-hidden="true" style="color: #5a0421;"></i>' +
        '</div>' +
        '</div><!--bot_profile end-->' +
        ...
        '<input type="text" id="chat-input" autocomplete="off" name="text_msg" placeholder="Start typing here..."' + 'class="form-control bot-txt" style="border: 0px; border-radius: 25px; background-color: #fff; padding-left: 5%; margin-left: 10px; margin-top: 10px; margin-bottom: 10px; height: 50px; box-shadow: 1px 1px 5px #ccc;"/>' +
        '<div class="input-group-append">' +
        '<button class="btn" type="button" style="background-color: #5a0421; margin-top:10px; margin-right: 5px; margin-left: 8px; border-radius: 50%; width: 50px; height: 50px"><i class="fa fa-paper-plane" aria-hidden="true" style="color: #fff; font-size: 1.5rem;"></i></button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div><!--chatCont end-->' +
    ...
    function send(text) {
        ...
        $.ajax({
            ...
            url: 'https://5005-fuchsia-guineafowl-cjiolvvi.ws-eu16.gitpod.io/webhooks/rest/webhook', //  RASA API
            type: 'POST',
            headers: {
    ...
    function setBotResponse(val) {
        setTimeout(function() {
            ...
            if ($.trim(val) == '' || val == 'error') { //if there is no response from bot or there is some error
                val = 'Sorry! I did not understand you, please repeat once again.'
